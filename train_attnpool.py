...
class AttentionPoolHead(nn.Module):
    """Learned attention over patch tokens, then the same MLP as the baseline.

    The baseline hands the head one vector: CLIP's own pooled embedding, which
    was learned for "what is in this picture". Quality is often local — one
    corner is soft while the rest is sharp — and that pooling can average the
    evidence away. Here the head sees all N patch tokens and learns its own
    weights over them. The extra cost is one 768->1 linear, 769 parameters
    against the baseline's 198,657, so a change in accuracy is the structure
    talking, not the capacity.
    """
...
def embed(backbone, images: torch.Tensor) -> torch.Tensor:
    return backbone(pixel_values=images).last_hidden_state.float()
...
def main() -> None:
    ...
    head = AttentionPoolHead(feature_dim, args.hidden_dim).to(device)
